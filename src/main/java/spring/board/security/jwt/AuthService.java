...
public class AuthService {
    ...
    public UserTokenDto login(UserResponseDto userResponseDto) {
        ...
        // 4. RefreshToken 저장
        RefreshToken refreshToken = RefreshToken.builder().key(authentication.getUsername())
    ...
    public UserTokenDto reissue(UserTokenDto oldToken) {
        // 1. Refresh Token 검증
        if (!tokenProvider.validateToken(oldToken.getRefreshToken())) throw new RuntimeException("Refresh Token 이 유효하지 않습니다.");
        ...
        if (!refreshToken.getValue().equals(oldToken.getRefreshToken())) throw new RuntimeException("토큰의 유저 정보가 일치하지 않습니다.");
